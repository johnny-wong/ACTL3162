...
# estimating variance function to check for over-dispersion
#
# read the data file
...
# glm (poisson) models estimated using quasi-likelihood functions

 pcarins.glm <- glm(Numb.Claims~Pol.Age+Car.Group+Veh.Age, family=quasi(link="log",variance="mu"))
# pcarins.glm <- glm(Numb.Claims~Pol.Age+Car.Group+Veh.Age, family=poisson)
print(summary(pcarins.glm))
print(anova(pcarins.glm, test="F")) #F because of quasi
