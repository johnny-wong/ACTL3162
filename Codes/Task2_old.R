...
g_discrete <- function(x, h, m, shape, rate){
  lower_bound <- max(0, x - h/2)
  upper_bound <- min(h*m, x + h/2)
  if ((((x/h) %% 1) != 0)|
    (x < 0) | (x > h*m)){
    mass <- 0
  } else if (lower_bound == 0){
    mass <- pgamma(upper_bound, shape = shape, rate = rate)
  } else if (upper_bound == h*m){
    mass <- 1 - pgamma(lower_bound, shape = shape, rate - rate)
  } else {
    mass <- pgamma(upper_bound, shape = shape, rate = rate) -
      pgamma(lower_bound, shape = shape, rate = rate)
  }
  return(mass)
}
...
# Create a function for G*, the discretisation of G
G_discrete <- function (x, h, m, shape, rate){
  # Find corresponding discrete level, nearest multiple of h less than x
  x_discrete <- floor(x/h)*h
  
  if (x_discrete >= h*m ){
    G <- 1
  } else
    G <- pgamma(x_discrete + h/2, shape = shape, rate = rate)
  return(G)
}
...
for (h in c(1, 5)){
  for (m in c(150, 300)/h){
    for (init_capital in v_c0){
      ...
      psi_2 <- 0
      for (i in sum_index){
        psi_2 <- psi_2 + g_discrete(i, h, m, shape = alpha, rate = beta) * 
          psi_1(c0 + premium - i)
      }
